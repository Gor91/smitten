...
class RestSerializer extends Component
{
        ...
        if ($data instanceof Model && $data->hasErrors()) {
            ...
            foreach ($data as $key => $model) {
                if ($model instanceof Arrayable) {
                    $data[$key] = $this->serializeModel($model);
                } elseif ($model instanceof DataProviderInterface) {
                    $data[$key] = $this->serializeDataProvider($model);
                }
            }
        }
    ...
    {
        return strtolower(basename(substr(strrchr($class::className(), "\\"), 1)));
    }
}